...
const OrderSummry = () => {
  ...
  const handlePlaceOrder = () => {
    const orderData = { orderItems: cartItems, shippingAddress: address, paymentMethod: paymentMethod, itemsPrice: cartPrice, totalprice: totalPrice, shippingPrice: deleviryPrice, email: user.eamil, userName: user.name }
    dispatch(placeOrder(orderData))

    let oldOrders = localStorage.getItem("orderList");
    oldOrders = JSON.parse(oldOrders)
    localStorage.setItem("orderList", JSON.stringify({ ...oldOrders, ...orderData }));
    navigate('/')
